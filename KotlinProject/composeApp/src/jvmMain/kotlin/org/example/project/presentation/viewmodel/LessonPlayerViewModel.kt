...
class LessonPlayerViewModel : ViewModel() {
    ...
    val canGoNext: Boolean
        get() = currentQuestion?.let { question ->
            ...
            when (question.questionType) {
                ...
                else -> answer != null && isChecked && when (answer) {
                ...
                repository.getLessonById(lessonId, includeQuestions = true)
                    .onSuccess { lesson ->
                        ...
                        lesson.questions.forEachIndexed { index, question ->
                            println("[LessonPlayer]   ${index + 1}. ${question.questionType.displayName}: ${question.questionText.take(50)}...")
                            println("[LessonPlayer]      - wrongAnswerFeedback: ${if (question.wrongAnswerFeedback.isNullOrBlank()) "NOT SET" else "\"${question.wrongAnswerFeedback}\""}")
                            println("[LessonPlayer]      - explanation: ${if (question.explanation.isNullOrBlank()) "NOT SET" else "\"${question.explanation}\""}")
    ...
    fun answerMultipleChoice(questionId: String, choiceId: String, isCorrect: Boolean) {
    ...
    fun answerIdentification(questionId: String, answer: String, correctAnswer: String) {
    ...
    fun answerParaphrasing(questionId: String, answer: String) {
    ...
    fun answerVoiceRecording(questionId: String, recordingUrl: String) {
    ...
    fun answerMatching(questionId: String, matches: Map<String, String>) {
        ...
        val isCorrect = question?.let { q ->
            ...
            if (matches.isEmpty() || matches.size != leftItems.size) {
                ...
                val allCorrect = matches.all { (leftId, rightId) ->
                    ...
                    val pairMatch = leftChoice?.matchPairId?.let { leftPairId ->
    ...
    fun submitLesson(userId: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val answers = lesson.questions.mapNotNull { question ->
                    val answer = _userAnswers.value[question.id]
                    answer?.let {
                        when (it) {
                            is UserAnswer.MultipleChoice -> {
                                println("[LessonPlayer]   Q${lesson.questions.indexOf(question) + 1}: Multiple Choice - choice=$it.choiceId, correct=$it.isCorrect")
                            ...
                            is UserAnswer.Identification -> {
                                println("[LessonPlayer]   Q${lesson.questions.indexOf(question) + 1}: Text Entry - answer=${it.answer.take(30)}..., correct=$it.isCorrect")
                                UserQuestionAnswerCreate(
                                    userId = userId,
                                    questionId = question.id,
                                    lessonId = lesson.id,
                            ...
                            is UserAnswer.VoiceRecording -> {
                                println("[LessonPlayer]   Q${lesson.questions.indexOf(question) + 1}: Voice Recording - url=$it.recordingUrl")
                                UserQuestionAnswerCreate(
                                    userId = userId,
                                    questionId = question.id,
                                    lessonId = lesson.id,
                            ...
                            is UserAnswer.Matching -> {
                                println("[LessonPlayer]   Q${lesson.questions.indexOf(question) + 1}: Matching - matches=${it.matches.size} pairs, correct=$it.isCorrect")
                                UserQuestionAnswerCreate(
                                    userId = userId,
                                    questionId = question.id,
                                    lessonId = lesson.id,
                ...
                val request = SubmitLessonAnswersRequest(
                    userId = userId,
                    lessonId = lesson.id,
                    answers = answers
                )
                ...
                repository.submitLessonAnswers(request)
                    .onSuccess { result ->
                        ...
                        println("[LessonPlayer] Result: score=${result.score}%, correct=${result.correctAnswers}/${result.totalQuestions}, passed=${result.isPassed}")
