...
class AudioPlayer {
     ...
     */
    suspend fun playAudioFromUrl(audioUrl: String): Result<Unit> =
        withContext(Dispatchers.IO) {
            try {
                ...
                try {
                    ...
                    val downloadSuccess =
                        if (audioUrl.contains("supabase.co/storage/v1")) {
                    ...
                    // Detect MP3 format
                    val isMp3 =
                        (headerBytes == 0xFF && (secondByte == 0xFB || secondByte == 0xF3 || secondByte == 0xFA || secondByte == 0xF2)) ||
     ...
     */
    private suspend fun downloadFromSupabase(
        audioUrl: String,
        tempFile: File,
    ): Boolean {
        return try {
     ...
     */
    private suspend fun downloadFromDirectUrl(
        audioUrl: String,
        tempFile: File,
    ): Boolean {
        return try {
                ...
                for (fallbackFormat in fallbackFormats) {
                    try {
                        ...
                        if (AudioSystem.isLineSupported(fallbackInfo)) {
                            println(
                                "Using fallback format: ${fallbackFormat.sampleRate}Hz, ${fallbackFormat.sampleSizeInBits}bit, ${fallbackFormat.channels}ch",
                            )
     ...
     */
    suspend fun playAudioFromFile(filePath: String): Result<Unit> =
        withContext(Dispatchers.IO) {
            try {
