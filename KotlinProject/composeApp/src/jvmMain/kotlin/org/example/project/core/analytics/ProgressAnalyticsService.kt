...
object ProgressAnalyticsService {
    ...
    suspend fun getLanguageProgress(
        ...
        withContext(Dispatchers.IO) {
            ...
            val lessonsDeferred = async {
                repository.getLessonsProgress(userId, language).getOrElse { 
                    println("[ProgressAnalytics] ⚠️ Lessons progress failed, using default")
                    org.example.project.data.repository.LessonsProgress(0, 0)
                }
            }

            val sessionsDeferred = async {
                repository.getConversationSessions(userId, language).getOrElse { 
                    println("[ProgressAnalytics] ⚠️ Sessions count failed, using 0")
                    0 
                }
            }

            val vocabularyDeferred = async {
                repository.getVocabularyCount(userId, language).getOrElse { 
                    println("[ProgressAnalytics] ⚠️ Vocabulary count failed, using 0")
                    0 
                }
            }

            val scoresDeferred = async {
                repository.getVoiceAnalysisScores(userId, language).getOrElse { 
                    println("[ProgressAnalytics] ⚠️ Voice scores failed, using empty")
                    VoiceAnalysisScores() 
                }
            }

            val timeDeferred = async {
                repository.getTotalConversationTime(userId, language).getOrElse { 
                    println("[ProgressAnalytics] ⚠️ Conversation time failed, using 0")
                    0.0 
                }
            }
            ...
            println("[ProgressAnalytics] 📊 Stats: ${lessons.completed}/${lessons.total} lessons, " +
    ...
    suspend fun getMultiLanguageProgress(
        ...
        forceRefresh: Boolean = false
    ): Map<LessonLanguage, Result<LanguageProgress>> = withContext(Dispatchers.IO) {
     ...
     */
    fun invalidateCache(userId: String, language: LessonLanguage? = null) {
        if (language != null) {
            val removed = cache.remove(CacheKey(userId, language))
            println("[ProgressAnalytics] 🗑️ Invalidated cache for ${language.displayName} (user: $userId): ${removed != null}")
        } else {
            // Invalidate all languages for this user
            val keysToRemove = cache.keys.filter { it.userId == userId }
            keysToRemove.forEach { cache.remove(it) }
            println("[ProgressAnalytics] 🗑️ Invalidated all language caches for user: $userId")
        }
    }
    ...
    fun clearAllCache() {
        val size = cache.size
        cache.clear()
        println("[ProgressAnalytics] 🗑️ Cleared entire cache ($size entries)")
    }
    ...
    fun getCacheStats(): CacheStats {
        val currentTime = System.currentTimeMillis()
        val entries = cache.entries.map { (key, cached) ->
            CacheEntry(
                userId = key.userId,
